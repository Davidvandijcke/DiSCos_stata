                                ...
                                real scalar T0, real scalar T_max) 
{
    ...
    if (cols(sample_points) < 1) {
        errprintf("sample_points must contain at least one value\n")
        return(1)
    }
    
    // Sort sample points and ensure they include 0 and 1
    sample_points = sort(sample_points', 1)
    if (sample_points[1] > 0) {
        sample_points = 0 \ sample_points
    }
    if (sample_points[length(sample_points)] < 1) {
        sample_points = sample_points \ 1
    }
    
    // Number of intervals
    real scalar n_intervals
    n_intervals = length(sample_points) - 1
    
    // Initialize summary stats matrix
    // Columns: time period, interval start, interval end, effect, se, ci_lower, ci_upper
    real matrix summary_stats
    summary_stats = J(n_intervals * (T_max - T0 + 1), 7, .)
    
    // Determine which matrices to use based on aggregation type
    real matrix data, data_boot
    real scalar grid_length
    
    if (agg == "quantileDiff" | agg == "quantile") {
        data = quantile_diff
        data_boot = quantile_diff_boot
        grid_length = rows(quantile_diff)
    }
    else {
        data = cdf_diff
        data_boot = cdf_diff_boot
        grid_length = rows(cdf_diff)
    }
    
    // Row counter for summary stats
    real scalar row
    row = 1
    ...
    for(t = T0; t <= T_max; t++) {
        ...
        for(i = 1; i <= n_intervals; i++) {
            real scalar q_start, q_end
            q_start = sample_points[i]
            q_end = sample_points[i + 1]
            
            // Find indices for this interval
            real vector idx
            idx = find_grid_indices(q_start, q_end, grid_length)
            
            // Compute effect
            real scalar effect
            effect = compute_mean_effect(data, idx[1], idx[2], t)
            
            // Store basic results
            summary_stats[row, 1] = t
            summary_stats[row, 2] = q_start
            summary_stats[row, 3] = q_end
            summary_stats[row, 4] = effect
            
            // Compute bootstrap statistics if CI requested
            if (CI) {
                real vector boot_stats
