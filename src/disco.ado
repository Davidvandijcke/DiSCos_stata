...
Options:
    ...
    SIMPLEX:           Constrain weights to unit simplex
...
program define disco, eclass
    ...
    // Clear Mata and include required files
    mata: mata clear
...
//         if ("`agg'" != "") {
//             string scalar samples_str
//             samples_str = "`samples'"
//             real vector sample_points
//             sample_points = strtoreal(tokens(samples_str))
//            
//             rc3 = compute_summary_stats("`agg'", sample_points, T0, T_max)
//         };
    }
    
    // Generate plots if requested
    if "`graph'" != "" & "`agg'" != "" {
        if "`agg'" == "quantile" | "`agg'" == "quantileDiff" {
            local ytitle "Quantile"
            local title "Quantile Function"
        }
        else {
            local ytitle "CDF"
            local title "Cumulative Distribution Function"
        }
        
        if `doci' {
            twoway (rarea qdiff_lower qdiff_upper evgrid if t >= T0, color(gs12)) ///
                   (line quantile_diff evgrid if t >= T0, lcolor(black)), ///
                   by(t, note("")) title("`title'") ytitle("`ytitle'") ///
                   xtitle("Evaluation Points") legend(off)
        }
        else {
            twoway (line quantile_diff evgrid if t >= T0, lcolor(black)), ///
                   by(t, note("")) title("`title'") ytitle("`ytitle'") ///
                   xtitle("Evaluation Points") legend(off)
        }
    }
    ...
    // Display immediate results if aggregation was requested
    if "`agg'" != "" {
        disco_estat summary
    }
